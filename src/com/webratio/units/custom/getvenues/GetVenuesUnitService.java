package com.webratio.units.custom.getvenues;
...
public class GetVenuesUnitService extends AbstractService implements RTXOperationUnitService{

	private String url = "https://api.3cixty.com/v2/executeQuery";
	private String key = "78710ad5-6a97-4b1e-94d3-571ba508e6b7";
	private String query = "SELECT DISTINCT (?place AS ?uri) ?placeName (COUNT(DISTINCT ?event) AS ?count) " +
			"WHERE { GRAPH <{graph}> { ?event a lode:Event . } ?event lode:atPlace ?place . ?place rdfs:label ?placeName . } " +
    ...
    public GetVenuesUnitService(String id, RTXManager mgr, Element descr) throws RTXException {
        super(id, mgr, descr);
...
;		// return bean;
		...
		ResultVenueObject result = new ResultVenueObject();
